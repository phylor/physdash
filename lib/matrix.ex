defmodule Matrix do
  ...
  def with_size(columns, rows, default_value) do
    from_list(Enum.map(1..rows, fn _ ->
      Enum.map(1..columns, fn _ -> default_value end)
    end))
  end
  ...
  def flip(array_of_arrays) do
    [first_row | _] = array_of_arrays

    first_row |> Enum.with_index |> Enum.map fn {_column_value, column_index} ->
      Enum.map array_of_arrays, fn row -> Enum.at(row, column_index) end
    end
  end
end
